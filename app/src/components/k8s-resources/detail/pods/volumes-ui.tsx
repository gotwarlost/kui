...
export class Volumes extends React.Component<IVolumeProps, {}> {
    public render() {
        const rows = (this.props.volumes || []).map( (vol) => {
            ...
            if (vField === "configMap") {
                vField = <ObjectLink type={StandardResourceTypes.CONFIG_MAP} name={copy[vType].name}
                                     namespace={this.props.namespace}>configMap</ObjectLink>;
            } else if (vField === "secret") {
                vField = <ObjectLink type={StandardResourceTypes.SECRET} name={copy[vType].secretName}
